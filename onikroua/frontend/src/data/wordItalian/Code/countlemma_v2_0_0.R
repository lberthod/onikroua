...
for(myfile in file.names){
  ...
  nouns=themfiles
  # remove nouns with _noun tag in the end
  nouns$form <- gsub(pattern = "_NOUN", x = nouns$form, replacement = '',ignore.case = F)
  nouns$form <- gsub(pattern = "NOUN", x = nouns$form, replacement = '',ignore.case = F)
  ...
  setwd(wd_out)
  write.csv(toktyp, file = paste(thenumber, "_nounlist.csv", sep=""))
  }
...
summary(dat0)
dat0$X=NULL
dat0=dat0[dat0$POS=="NOUN", ]
dat0$POS=as.factor(dat0$POS)

# delete all occurrencies of words whose length is <1 or >20
dat0$Form=as.character(dat0$Form)
dat0 = dat0[ which((nchar(dat0$Form)>1) & (nchar(dat0$Form)<25)), ]
dat0$Form=as.factor(dat0$Form)

#check! lemma frequency should be 1 or 2. Vaues > 2 can derive from reduplication of entries
lemmacheck=(as.data.frame(table(dat0$lemma)))
any(lemmacheck$Freq>2)

# Inspect possible problem cases (lemma):
# some lemmafrequencies >2 are due to tagging of different orthographic forms
#kazako=dat0[dat0$lemma=="kazako", ] #contains 'kazakhi', 'kazaki', 'kazako' word forms
...
# some lemmafrequencies >2 are due to morphologically syncretic lemmas
#hockeysta=dat0[dat0$lemma=="hockeysta", ] #contains 'hockeysta' 'hockeyste' 'hockeysti' word forms
...
setwd(wd_list)
write.csv(dat_ord0, file="itwac_nouns_lemmas_raw_2_0_0.csv", row.names = F)
write.csv(dat_ord1, file="itwac_nouns_lemmas_notail_2_0_0.csv", row.names = F)
...
datcheck_0=read.csv("itwac_nouns_lemmas_raw_2_0_0.csv", dec=".", sep=",", header=T)
head(datcheck_0)
tail(datcheck_0)
summary(datcheck_0)
str(datcheck_0)
...
datcheck_1=read.csv("itwac_nouns_lemmas_notail_2_0_0.csv", dec=".", sep=",", header=T)
head(datcheck_1)
tail(datcheck_1)
summary(datcheck_1)
str(datcheck_1)
