...
#This was adapted to VERB tagging, which includes some information about modality, such as "VER:infi", "VER:fin" 
...
for(myfile in file.names){
  ...
  nouns2= nouns2[startsWith(nouns2$POS, "VER"), ]
  ...
  setwd(wd_out)
  write.csv(toktyp, file = paste(thenumber, "_verblist.csv", sep=""))
  }
...
setwd(wd_list)
write.csv(dat_ord0, file="itwac_verbs_lemmas_raw_2_1_0.csv", row.names = F)
write.csv(dat_ord1, file="itwac_verbs_lemmas_notail_2_1_0.csv", row.names = F)
write.csv(lemmatypes, file="itwac_verbs_list_of_lemmas_2_1_0.csv", row.names = F)
...
datcheck_0=read.csv("itwac_verbs_lemmas_raw_2_0_0.csv", dec=".", sep=",", header=T)
head(datcheck_0)
tail(datcheck_0)
summary(datcheck_0)
str(datcheck_0)
...
datcheck_1=read.csv("itwac_verbs_lemmas_notail_2_0_0.csv", dec=".", sep=",", header=T)
head(datcheck_1)
tail(datcheck_1)
summary(datcheck_1)
str(datcheck_1)
